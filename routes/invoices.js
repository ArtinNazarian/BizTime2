...
router.put("/:id", async (req, res, next) => {
  try {
    ...
    const isPaid = paidResult.rows[0].paid;
    const paidDate = paidResult.rows[0].paid_date;
    ...
    const result = await db.query(
      "UPDATE invoices SET amt=$1, paid=$2, paid_date=$3 RETURNING id, comp_code, amt, paid, add_date, paid_date",
      [amt, paid, paidDate]
    );
